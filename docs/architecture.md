...
`modulePathIgnorePatterns`, and `resolver` replace configured values from each
owning project root before worker requests are built. Ignored paths are removed
both from dependency-graph discovery and from resolved dependency edges, so
changed-file selection cannot reach an invisible module. Extensions admitted
