...
- An automated differential denominator containing executable compatibility
  probes. The current generated result is 194/194 compatible scenarios (100.0%),
  ...
  environments at 7/7 (100.0%), coverage at 7/7 (100.0%), reporters at 6/6
...
entries. The current probe corpus score is 194/194 (100.0%). Core API is 13/13
(100.0%), ESM is 8/8
...
coverage is 7/7 (100.0%) in their bounded scenario categories. Fake timers are
...
- Inline-object and string/path-glob entries in Jest `projects` are normalized
  ...
  built-in reporters, and path/glob threshold edges remain incomplete. The
  latest Apollo capture proves the covered
...
- V8 coverage, non-global threshold groups, watch plugins, Mercurial/Sapling
  changed-file selection, exact custom-environment VM-context identity, worker
...
5. Stream test-case reporter events during worker execution and expand
   path/glob threshold differentials.
