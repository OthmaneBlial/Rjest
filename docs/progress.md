...
- An automated differential denominator containing executable compatibility
  probes. The current generated result is 109/109 compatible scenarios (100.0%),
  with Core API at 11/11 (100.0%), ESM at 8/8
  (100.0%), transforms at 5/5 (100.0%), mocks at 13/13 (100.0%), configuration
  at 23/23 (100.0%), CLI at 6/6 (100.0%), environments at 7/7 (100.0%), and
...
- Apollo Client 4.2.12 is pinned as a current Jest 30, TypeScript, React
  ...
  passing, nine failing, 476 skipped, and 519/519 snapshots matched. After the
  ESM TypeScript config fix, Rjest reaches and explicitly rejects the
  unsupported six-entry `projects` field; no Apollo tests execute yet.
...
entries. The current probe corpus score is 109/109 (100.0%). Core API is 11/11
(100.0%), ESM is 8/8
(100.0%), transforms are 5/5 (100.0%), mocks are 13/13 (100.0%), configuration
is 23/23 (100.0%), CLI is 6/6 (100.0%), resolution is 10/10 (100.0%), snapshots
are 10/10 (100.0%), environments are 7/7 (100.0%), and coverage is 3/3 in
their bounded scenario categories.

## Known blockers

- Jest `projects` configuration is not yet normalized or scheduled. Apollo
  Client's six inline project configs are the current implementation target;
  Rjest intentionally rejects them before discovery instead of running only a
  misleading root subset.
...
1. Implement inline Jest `projects` configuration and converge Apollo Client's
   six-project suite from zero executed tests toward its 9,974-test oracle.
2. Grow the versioned differential denominator from deterministic Apollo
   failures.
