...
- An automated differential denominator containing executable compatibility
  probes. The current generated result is 192/192 compatible scenarios (100.0%),
...
entries. The current probe corpus score is 192/192 (100.0%). Core API is 13/13
(100.0%), ESM is 8/8
...
10/10 (100.0%), reporters are 6/6 (100.0%), and Watch is 3/3 (100.0%).
...
- V8 coverage, non-global threshold groups, interactive watch controls/plugins,
  stale-run cancellation, Mercurial/Sapling changed-file selection, exact
  custom-environment VM-context identity, worker reuse, and run-wide Ctrl+C
  cancellation are not implemented. `--watchAll` and Git-aware `--watch`
  lifecycle parity are covered.
...
1. Add stale-run cancellation and interactive failed/all/path/name rerun
   controls to the native watch cycle, then cover Mercurial/Sapling selection.
2. Add another independent native-ESM/TypeScript monorepo corpus and reduce its
   highest-impact incompatibility classes against official Jest.
3. Add a strict pnpm isolated-linker corpus and broaden configured-resolver
