...
- An automated differential denominator containing executable compatibility
  probes. The current generated result is 152/152 compatible scenarios (100.0%),
  ...
  environments at 7/7 (100.0%), coverage at 4/4 (100.0%), and
...
entries. The current probe corpus score is 152/152 (100.0%). Core API is 13/13
(100.0%), ESM is 8/8
...
- Inline-object and string/path-glob entries in Jest `projects` are normalized
  ...
  transform and discovery caches, display-color, and reporter/threshold edges
  remain incomplete. The latest Apollo capture proves the covered
...
4. Expand persisted transform caching and cross-project reporter/threshold
   differentials.
