...
- An automated differential denominator containing executable compatibility
  probes. The current generated result is 151/151 compatible scenarios (100.0%),
...
entries. The current probe corpus score is 151/151 (100.0%). Core API is 13/13
(100.0%), ESM is 8/8
...
- Inline-object and string/path-glob entries in Jest `projects` are normalized
  ...
  Multi-project coverage, persisted transform and discovery caches,
  display-color, and reporter edges remain incomplete. The latest Apollo
...
4. Expand multi-project coverage, persisted transform caching, and reporter
   differentials.
