...
- An automated differential denominator containing executable compatibility
  probes. The current generated result is 101/101 compatible scenarios (100.0%),
...
entries. The current probe corpus score is 101/101 (100.0%). Core API is 11/11
(100.0%), ESM is 7/7
(100.0%), transforms are 5/5 (100.0%), mocks are 13/13 (100.0%), configuration
is 21/21 (100.0%), CLI is 6/6 (100.0%), resolution is 7/7 (100.0%), snapshots
...
- Synchronous and async-only custom resolver exports work across the covered
  CommonJS/native-ESM paths. Complete mutable Jest resolver options and
  package-manager-specific PnP behavior remain open.
...
2. Grow the versioned differential denominator from deterministic real-project
   failures.
3. Differential-test custom resolver package filtering and PnP boundaries.
