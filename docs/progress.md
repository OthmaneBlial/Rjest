...
- An automated differential denominator containing both passing probes and
  preserved known incompatibilities. The current generated result is 71/72
  compatible scenarios (98.6%), with ESM at 7/7 (100.0%), transforms at 5/5
  (100.0%), mocks at 10/11 (90.9%), and per-category scores in the machine-readable
...
corpus score is 71/72 (98.6%). ESM is 7/7 (100.0%), transforms are 5/5
(100.0%), mocks are 10/11 (90.9%), and coverage is 3/3 in their bounded scenario
categories.
...
- Deeply unmocked dependency graphs are not implemented. Manual
  CommonJS/native-ESM mocks and CommonJS/native-ESM
...
2. Grow the versioned differential denominator from deterministic real-project
   failures.
3. Implement transitive CommonJS `jest.deepUnmock` semantics.
