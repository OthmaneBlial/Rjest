...
- Mock return-history matchers cover nth and last successful returns, thrown and
  incomplete calls, asymmetric values, and positive-integer validation.
- Jest 30 asymmetric factories cover `expect.arrayOf`, `expect.closeTo`, their
  negative forms, custom equality testers, strict input validation, special
  `expect.any` constructors, and Jest's empty `arrayContaining` behavior.
- `jest.isEnvironmentTornDown()` follows the runtime/environment teardown
  boundary and remains observable from a custom environment's teardown hook.
...
- Default and JSON output plus stateful CommonJS/ESM custom reporters with
  awaited run/file/case hooks, legacy fallbacks, multi-project contexts,
  custom-only output selection, and `getLastError()` failure codes.
- Persistent Jest `globalSetup`/`globalTeardown` execution for CommonJS, native
  ESM, and transformed TypeScript modules. Async hooks share coordinator-side
  state, setup environment changes reach reporters/workers, shared paths are
  deduplicated across active projects, filtered projects stay inactive, and
  teardown/error exits follow the covered Jest lifecycle.
- Awaited CommonJS and ESM `testResultsProcessor` execution after global
  teardown, with Jest-shaped aggregate counts and test results, setup/teardown
  environment visibility, processed JSON fields, CLI overrides, and
  processor-controlled failures.
- Project `verbose` configuration now reaches native output, custom reporters,
  and global-hook `globalConfig`, removing the adjacent blocker in unchanged
  Granite Detox configurations.
...
- Jest 30 `setTimerTickMode` supports manual, next-async, and interval clock
  drivers, mode switching, configured auto-advance cancellation, and legacy
  timer rejection. Explicit async advancement temporarily pauses next-async
  mode so two drivers cannot race or move the fake clock backwards.
...
- Combined multi-project bail sequencing after sharding, retaining each test's
  owning project configuration while applying Jest's uncached
  larger-file-first fallback across the complete matrix. A marker differential
  proves that an earlier configured passing project is not executed after a
  larger failing test in a later project reaches bail.
- Configured custom Jest test sequencers through a persistent Node lifecycle
  bridge. CommonJS and native-ESM classes receive project contexts and global
  seed/shard data; sync/async `shard` runs before sync/async `sort`; the ordered
  identities retain their owning project configuration; and the same instance
  receives Jest-shaped aggregate data through `cacheResults`. Differential
  markers cover reverse execution, custom shard membership, inherited
  `@jest/test-sequencer` caching, ESM cache finalization, and CLI
  `--testSequencer` precedence over configuration.
- Jest-compatible `--onlyFailures`/`-f` selection through synchronous or
  asynchronous custom `allFailedTests` hooks and a native persisted default
  performance cache. Default ordering prioritizes cached failures, uncached
  files, and longer cached files. Cold caches, post-bail behavior, fully skipped
  suites, assertion failures, and zero-test file execution errors are preserved
  as differential regressions.
- Jest cache controls for the sequencer lifecycle: normalized `cache` and
  `cacheDirectory`, CLI `--cache`/`--no-cache`, and `--clearCache` across
  distinct project directories. Differentials prove config disablement, CLI
  precedence, failure-history invalidation, configured-directory deletion, and
  symmetric cold-cache oracle runs. Broad destructive targets are refused.
...
- Jest 30 mock functions now expose `whenCalledWith` argument routing and
  `getMockImplementation`, including asymmetric and iterable equality,
  overlapping-branch precedence, parent and branch one-shot queues, fallbacks,
  call state, constructors, spies, promise helpers, resets, and temporary
  implementations.
- Explicit `testRunner: "jest-circus/runner"` configuration and resolved
  installed Circus paths are accepted as the lifecycle Rjest implements.
  Custom runner modules remain rejected instead of being ignored.
- `--testRunner=jest-circus/runner` applies the same explicit Circus selection
  across every project in the execution matrix. CLI validation rejects custom
  runner values before execution.
- Jest-aligned core matcher contracts for numeric comparisons, same-sign
  infinities, constructor checks, length values, string containment, RegExp
  state, object matching, and property paths, preserved by a nine-test oracle
  fixture.
- Jest-aligned matcher usage errors for unexpected arguments, invalid mock
  counts, and non-mock receivers, plus call-matcher support for legacy
  Jasmine-compatible spy records, preserved by an eight-test oracle fixture.
- Jest-aligned `toThrow` semantics for `throw undefined`, RegExp-like objects,
  unsupported expectations, nested error causes, custom error constructors,
  and non-Error thrown values, preserved by an eight-test oracle fixture.
- Modern fake timers accept Temporal-style epoch and duration objects, keep
  `Date` callable with or without `new`, set epoch zero when `setSystemTime()`
  omits its value, and reject negative advances in sync and async modes. An
  eight-test oracle fixture preserves these contracts alongside legacy timers.
- Timer controls called while real timers are active now match Jest's warning
  and no-op contract instead of throwing, including synchronous/asynchronous
  advances, drains, clock mutation, timer counts, and public return values. A
  six-test oracle fixture also preserves silent clearing and legacy warnings.
- Modern `timerLimit` now matches Jest/Sinon's recursive timer and microtask
  boundaries, exact infinite-loop error, runtime override, and async drain
  behavior without incorrectly capping finite sync/async time advances. A
  six-test oracle fixture preserves callback counts as well as pass/fail state.
- Modern Node fake timers now return Sinon-compatible timeout, interval, and
  immediate handles with ref state, numeric coercion, refresh/rescheduling,
  equal-deadline ordering, and stale-clock isolation. JSDOM retains numeric
  browser handles; an eight-test oracle fixture preserves both environments.
- Modern clear APIs forward pre-existing native timeout, interval, and immediate
  handles to their captured Node/JSDOM implementations, return `undefined`,
  preserve timeout/interval cross-clearing, and reject incompatible immediate
  clears with Sinon's diagnostic. An eight-test oracle fixture preserves the
  host boundary and both environments.
- Modern `runOnlyPendingTimers` now runs intervals and newly scheduled work up
  to the original last deadline, leaves later work pending, and drains fake
  microtasks even when no timer exists. The async form preserves the same
  boundary across native promise turns; a six-test oracle fixture covers both.
- Modern timer delays now clamp above `2^31 - 1`, preserve that exact maximum,
  parse string-like delays like Sinon, and retain zero-time behavior for
  non-finite values. Missing and invalid callbacks use Sinon's diagnostics; a
  six-test oracle fixture covers timeout, interval, and immediate scheduling.
- Modern JSDOM animation frames now share Sinon's callback validation, reject
  timeout/frame cross-clearing with Jest's exact API names, keep timers pending
  after invalid clears, and forward cancellation of native frames created
  before fake timers were installed. A four-test oracle fixture covers these
  contracts.
- Modern fake microtasks now defer invalid `process.nextTick` and
  `queueMicrotask` callbacks until drain time like Sinon. A failed job preserves
  and replays the complete queue, while successful drains include nested jobs
  and forward `nextTick` arguments; a four-test oracle fixture covers these
  contracts.
- Mock functions now support `withImplementation` across synchronous,
  asynchronous, and nested scopes. Temporary scopes isolate queued once-values,
  restore on successful completion, and retain the temporary implementation on
  thrown/rejected callbacks like Jest; a five-test oracle fixture covers the
  contract.
- Mock clear/reset/restore state now matches Jest: `lastCall` is absent until a
  call, reset and restore clear custom names, restore returns `undefined`, and
  `Symbol.dispose` delegates to restore for standalone mocks and spies. A
  five-test oracle fixture covers the boundaries.
- Runtime `expect.addSnapshotSerializer()` registration now matches Jest. It
  returns `undefined`, tests the latest plugin first, and supports legacy
  `print` plus modern `serialize` plugins with recursive printing. A three-test
  oracle fixture preserves the contracts.
- Direct `test.concurrent` children now run as one bounded suite unit under
  `maxConcurrency`, while sequential children wait and per-test hooks stay
  sequential-only. Async-local identity isolates concurrent assertions,
  snapshots, and environment events; a six-test oracle fixture covers overlap,
  grouping, hooks, and concurrent snapshots.
- `expect.getState()` now exposes Jest's async-local
  `currentTestIdentity()` and `currentConcurrentTestName()` callbacks.
  Concurrent identities stay distinct and stable across awaits, sequential
  hooks share the body identity, and suite hooks see `undefined`; a three-test
  oracle fixture preserves the state contract.
- The `expect` entry point now preserves Jest's zero-or-one-argument contract
  and exact extra-argument diagnostic instead of silently discarding values. A
  two-test oracle fixture covers both accepted forms and rejection cases.
- Tagged-template tables now work across `test.each`, `describe.each`, and
  `test.concurrent.each`. Named columns become row objects, nested name
  interpolation remains available, and concurrent rows keep real bounded
  overlap; a seven-test oracle fixture covers all three declaration families.
- `injectGlobals: false` now preserves Jest's explicit-import mode: framework
  APIs stay off `globalThis` while CommonJS `@jest/globals` imports retain
  suites, hooks, assertions, and the module-scoped Jest object. The worker
  protocol is now v26 and defaults remain injection-compatible.
- Configured `testFailureExitCode` now reaches the final process boundary.
  Failed test runs use the requested status while passing runs remain zero and
  configuration errors remain one; a failing oracle fixture preserves both
  the test result and exact exit code 7.
- `--testFailureExitCode` now overrides the configured status from the command
  line. The permanent CLI differential runs the same failing fixture with a
  configured status of 7 and proves that both Jest and Rjest exit with 9.
- `testLocationInResults` now records one-based test-declaration lines and
  columns in JSON output. A two-test oracle fixture compares exact top-level and
  nested positions, and the optional field travels over worker protocol v27.
- `--testLocationInResults` now enables those exact coordinates from the CLI.
  Its separate fixture omits the configuration option, so the differential
  proves the command-line switch changes observable JSON rather than merely
  being accepted.
- Common runtime overrides now accept Jest's `--detectOpenHandles`,
  `--no-detectOpenHandles`, `--injectGlobals[=BOOLEAN]`,
  `--no-injectGlobals`, and `--testTimeout`. Four differential scenarios prove
  open-handle reporter state, configured-timeout precedence, explicit imports
  with globals disabled, and propagation across two child projects.
- Open-handle detection still executes serially, but custom reporters retain
  Jest's requested `maxWorkers` value instead of receiving Rjest's internal
  effective worker count.
- `--env` and `--testEnvironment` now share one recursive environment override,
  and `--testEnvironmentOptions` validates and forwards a JSON object. Two CLI
  oracles replace a configured Node environment with JSDOM and verify the exact
  browser URL.
- Built-in JSDOM now exposes Jest's browser-global identity:
  `window === self === globalThis`, while its existing live document, storage,
  IndexedDB, constructor, timer, and realm bindings remain covered.
- Common setup and concurrency CLI overrides now accept `--globals`,
  `--maxConcurrency`, `--setupFiles`, and `--setupFilesAfterEnv`. Four strict
  oracles prove JSON replacement, bounded concurrent execution, pre-framework
  module replacement, post-framework module replacement, and recursive child
  project application.
- Mock-runtime CLI overrides now accept positive, explicit-boolean, and
  `--no-*` forms for `automock`, `clearMocks`, `resetMocks`, `restoreMocks`, and
  `resetModules`. Five activation fixtures and one combined negation fixture
  prove state transitions and configuration precedence against Jest.
- Jest CI snapshot safety now recognizes `--ci` and common CI environment
  signals, refuses unreviewed new snapshots, and lets explicit `--ci=false`
  override environment detection. Three differential fixtures compare exit
  status and the snapshot filesystem tree.
- Test-path selection now accepts Jest 29/30 `--testPathPattern(s)` spellings
  and `--runTestsByPath`. Positional and option patterns remain regular
  expressions, while the explicit by-path mode resolves literal files. Three
  differential scenarios lock the distinction using regex metacharacters.
- Discovery CLI overrides now replace `testMatch`, `testRegex`,
  `testPathIgnorePatterns`, and `roots` recursively. Four oracle fixtures prove
  exact selected identities and project-root-relative path normalization.
- Module-resolution CLI overrides now replace `moduleDirectories`,
  `modulePaths`, `moduleFileExtensions`, `moduleNameMapper`, and `resolver`
  recursively. Five oracle fixtures also prove that a custom configured file
  extension without a transform executes as CommonJS, as it does under Jest.
- `modulePathIgnorePatterns` now has the same recursive CLI override and makes
  ignored modules fully invisible to the dependency graph. A related-test
  oracle proves that neither scanned modules nor resolved edges can pull an
  ignored changed path back into selection.
- Transformation and snapshot CLI overrides now accept `transform`,
  `transformIgnorePatterns`, and `snapshotSerializers`. Three strict oracles
  prove JSON transformer replacement, ignored-source bypass, exact serialized
  snapshot output, and project-local `<rootDir>` expansion.
- Haste and reporter CLI overrides now replace their configured objects/lists.
  Two oracles prove platform-specific module selection and exact custom
  reporter lifecycle output. An attempted `extensionsToTreatAsEsm` CLI probe
  was excluded because official Jest 30.5.0 itself rejects that invocation.
- Failed built-in and custom assertions now expose Jest-compatible structured
  `matcherResult` data. The oracle covers equality metadata, the underlying
  positive `pass` value behind a negated failure, evaluated messages, and
  custom result fields.
- Assertion failures now share Jest's exported `JestAssertionError` identity,
  including `instanceof`, constructor naming, and the public `name === "Error"`
  contract for built-in and custom matchers.
- `numPassingAsserts` now counts only successful matcher outcomes while
  `assertionCalls` continues to count every invocation. Built-in, negated,
  caught-failure, asynchronous custom matcher, snapshot, state, and result
  paths share the same counter.
- `expect.extractExpectedAssertionsErrors()` now returns Jest-shaped exact and
  at-least-one assertion failures, preserves their declaration errors, resets
  local state, and clears the active test contract. `hasAssertions()` also
  rejects an unexpected argument.
- Asymmetric matchers now expose Jest's expected-type metadata for constructors,
  arrays, objects, strings, and numeric proximity. Custom asymmetric matchers
  report type `any` and preserve Jest's `matcher<arguments>` formatting.
- An automated differential denominator containing executable compatibility
  probes. The current generated result is 280/280 compatible scenarios (100.0%),
  with Core API at 16/16 (100.0%), ESM at 8/8
  (100.0%), transforms at 7/7 (100.0%), mocks at 17/17 (100.0%), configuration
  at 52/52 (100.0%), snapshots at 16/16 (100.0%), CLI at 76/76 (100.0%),
  environments at 7/7 (100.0%), Expect at 19/19 (100.0%), fake timers at 20/20
  (100.0%), coverage at 19/19 (100.0%), reporters at 7/7 (100.0%), Watch at
  4/4 (100.0%), and
...
- Live custom-reporter case events in worker protocol v24. Case starts and
  results now reach the persistent reporter process while a file is active,
  preserve output that lacks a trailing newline, retain Jest's skip/todo event
  boundary, and abort the worker on callback failure. A differential test makes
  each test body depend on the reporter's prior live marker, so replay after
  file completion cannot satisfy the regression.
- Jest-compatible `coverageProvider: "v8"` in worker protocol v25. Each worker
  captures precise V8 ranges without Babel instrumentation; the Rust
  coordinator merges raw process coverage before one `v8-to-istanbul`
  conversion, matching Jest's cross-worker branch-map behavior. Permanent
  probes cover two workers executing opposite branches, zero-hit
  `collectCoverageFrom` files, coverage thresholds, and source-map remapping
  from generated JavaScript to TypeScript.
...
- Default multi-project sharding over a combined execution matrix, hashing each
  test relative to its owning project root. A two-root differential preserves
  the case where global-root hashing selected the wrong project for shard one.
...
- JavaScript and TypeScript config handoff now omits `undefined` object
  properties instead of rejecting the entire configuration. A corpus audit
  found the pattern in unchanged Granite and Nx configs; an official-Jest
  differential fixture preserves `displayName`, `resolver`, and nested optional
  values at that process boundary.
- Jest CLI `--forceExit` and `--no-coverage` now parse unchanged, override
  config across project matrices, and reach extension `globalConfig`. A
  differential open-handle fixture proves matching successful exit plus
  reporter-observed `forceExit`/`collectCoverage` values.
- Native `--watchAll` starts its recursive watcher before the initial run,
  coalesces platform events, filters configured ignore patterns and generated
  outputs, and performs fresh discovery on every cycle. The official-Jest
  process differential covers initial success, failure and recovery after a
  dependency edit, new-suite discovery, suite deletion, and stable idle state.
- Git-aware `--watch` discovers staged, modified, deleted, and untracked files,
  then uses a Rust-owned inverse dependency graph to select affected suites on
  every cycle. The official-Jest process differential covers ordered module
  mapping, two-hop CommonJS dependencies, native ESM dependencies, unrelated
  changes, added/deleted test files, a clean tree, stable idle state, and the
  no-SCM exit boundary. Snapshot ownership and conservative fallback for an
  async-only custom resolver have dedicated Rust regression tests.
- One-shot `-o`/`--onlyChanged`, `--lastCommit`, `--changedSince`, and
  `--changedFilesWithAncestor` reuse the same graph with Jest's positional and
  `--all` precedence. `--findRelatedTests` now accepts explicit source paths
  for pre-commit workflows, including direct/transitive and test-path
  selection, no-test exits, `--passWithNoTests`, and source-narrowed coverage.
- TTY-aware watch controls for all/changed/failed selection, filename and test
  name prompts, one-shot snapshot updates, filter clearing, manual reruns, and
  clean exit. Actionable keys pressed during an active run interrupt that run
  without also applying the action, matching Jest's two-key interruption/rerun
  lifecycle. The Rust coordinator terminates blocked Node workers through a
  shared cancellation token; a PTY differential proves the behavior against
  official Jest. Filesystem events do not cancel active work because Jest's
  `startRun` ignores them while a run is already in progress.
- A pinned React Testing Library/React 19 corpus: exact 16/16 suites, all
  251/251 test identities and statuses (248 passed, three skipped), and 11/11
  snapshots with no checkout mutation. The first semantically passing Rjest
  run exposed an unreported rewrite of Jest 29's historical snapshot header;
  Rjest now accepts and preserves both recognized Jest v1 headers, with a
  pinned Jest 29.7 differential oracle.
- Jest-compatible coverage threshold groups: exact files and directory paths
  aggregate their assigned coverage, globs evaluate every matched file,
  overlapping groups all apply, grouped files leave the ordinary global
  bucket, and an empty global bucket falls back to the full map. Positive
  percentages, negative uncovered-count limits, and missing-path failures are
  preserved by eight differential scenarios and focused Rust tests. Relative
  keys use Jest's invocation-directory base even when `rootDir` differs.
...
entries. The current probe corpus score is 280/280 (100.0%). Core API is 16/16
(100.0%), ESM is 8/8
(100.0%), transforms are 7/7 (100.0%), mocks are 17/17 (100.0%), configuration
is 52/52 (100.0%), CLI is 76/76 (100.0%), resolution is 12/12 (100.0%), snapshots
are 16/16 (100.0%), Expect is 19/19 (100.0%), environments are 7/7 (100.0%), and
coverage is 19/19 (100.0%) in their bounded scenario categories. Fake timers are
20/20 (100.0%), reporters are 7/7 (100.0%), and Watch is 4/4 (100.0%).
...
- Global setup/teardown now covers the common lifecycle, transformed hooks,
  environment propagation, failures, and active-project deduplication. Because
  Rjest keeps test files process-isolated, arbitrary non-environment objects on
  the hook process global are not copied into test workers; exact cross-process
  object identity and async-only transformed hook dependencies remain outside
  the current proof.
- Inline-object and string/path-glob entries in Jest `projects` are normalized
  and scheduled. Multi-project default sharding and uncached bail sequencing
  are covered, as are configured custom-sequencer sort, shard, cache, and
  only-failure lifecycles plus native persisted performance ordering.
  Global `collectCoverageFrom` matching and merged summary output are covered
  across distinct project roots, including untested sources. Persisted
  transform and discovery caches, specialized built-in reporters, and broader
  platform-specific glob edges remain
  incomplete. The latest Apollo capture proves the covered
  inline-project execution at 563 suite executions
  and 9,974 test identities with zero Rjest-only failures, but not those
  unmeasured project configuration combinations.
...
- Watch plugins, Mercurial/Sapling changed-file selection, exact custom-
  environment VM-context identity, worker
  reuse, and run-wide Ctrl+C cancellation outside watch mode are not
  implemented. Core interactive watch controls and active-worker interruption,
  `--watchAll`, and Git-aware `--watch` lifecycle parity are covered.
...
3. Broaden watch prompt/output parity and extension support, then cover
   Mercurial/Sapling changed-file selection.
4. Add broader timer edge-case probes and modern timer tick-mode controls.
5. Broaden specialized reporter payload/built-in coverage and
   platform-specific path/glob coverage.
6. Replace per-file Node/JSDOM startup after correctness remains
