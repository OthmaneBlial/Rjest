...
- An automated differential denominator containing executable compatibility
  probes. The current generated result is 100/100 compatible scenarios (100.0%),
  with Core API at 11/11 (100.0%), ESM at 7/7
  (100.0%), transforms at 5/5 (100.0%), mocks at 13/13 (100.0%), configuration
  at 21/21 (100.0%), CLI at 6/6 (100.0%), and
...
entries. The current probe corpus score is 100/100 (100.0%). Core API is 11/11
(100.0%), ESM is 7/7
(100.0%), transforms are 5/5 (100.0%), mocks are 13/13 (100.0%), configuration
is 21/21 (100.0%), CLI is 6/6 (100.0%), resolution is 6/6 (100.0%), snapshots
are 10/10 (100.0%), and coverage is 3/3 in their bounded scenario categories.
...
- Synchronous custom resolver functions and `{sync}` objects work across the
  covered CommonJS/native-ESM paths. Async-only resolver exports, complete Jest
  resolver options, and package-manager-specific PnP behavior remain open.
- V8 coverage, non-global threshold groups, watch mode, complete custom
  environments, worker reuse, and run-wide Ctrl+C cancellation are not
  implemented.
...
2. Grow the versioned differential denominator from deterministic real-project
   failures.
3. Support async-only custom resolver exports in native-ESM resolution.
