...
- An automated differential denominator containing executable compatibility
  probes. The current generated result is 91/91 compatible scenarios (100.0%),
  with Core API at 11/11 (100.0%), ESM at 7/7
  (100.0%), transforms at 5/5 (100.0%), mocks at 13/13 (100.0%), configuration
  at 18/18 (100.0%), CLI at 6/6 (100.0%), and
...
entries. The current probe corpus score is 91/91 (100.0%). Core API is 11/11
(100.0%), ESM is 7/7
(100.0%), transforms are 5/5 (100.0%), mocks are 13/13 (100.0%), configuration
is 18/18 (100.0%), CLI is 6/6 (100.0%), snapshots are 8/8 (100.0%), and
...
- Project Prettier integration for inline snapshot formatting, V8 coverage,
  non-global threshold groups, watch mode, complete custom environments, worker
  reuse, and run-wide Ctrl+C cancellation are not implemented.
...
2. Grow the versioned differential denominator from deterministic real-project
   failures.
3. Add project Prettier integration to inline snapshot source writes.
