...
- An automated differential denominator containing executable compatibility
  probes. The current generated result is 174/174 compatible scenarios (100.0%),
...
entries. The current probe corpus score is 174/174 (100.0%). Core API is 13/13
(100.0%), ESM is 8/8
...
10/10 (100.0%), reporters are 6/6 (100.0%), and Watch is 1/1 (100.0%).
...
- V8 coverage, non-global threshold groups, dependency-aware `--watch`,
  interactive watch controls/plugins, stale-run cancellation, exact
  custom-environment VM-context identity, worker reuse, and run-wide Ctrl+C
  cancellation are not implemented. `--watchAll` lifecycle parity is covered.
...
1. Build dependency-aware `--watch` selection, stale-run cancellation, and
   interactive failed/all/path/name rerun controls on the native watch cycle.
