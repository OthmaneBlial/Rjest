...
- An automated differential denominator containing executable compatibility
  probes. The current generated result is 126/126 compatible scenarios (100.0%),
  with Core API at 13/13 (100.0%), ESM at 8/8
  (100.0%), transforms at 5/5 (100.0%), mocks at 14/14 (100.0%), configuration
  at 28/28 (100.0%), CLI at 6/6 (100.0%), environments at 7/7 (100.0%), and
...
entries. The current probe corpus score is 126/126 (100.0%). Core API is 13/13
(100.0%), ESM is 8/8
(100.0%), transforms are 5/5 (100.0%), mocks are 14/14 (100.0%), configuration
is 28/28 (100.0%), CLI is 6/6 (100.0%), resolution is 12/12 (100.0%), snapshots
...
- Inline object entries in Jest `projects` are normalized and scheduled, but
  ...
  display-color, and reporter edge remain incomplete. The latest Apollo capture
  proves the covered inline-project execution at 563 suite executions and 9,974
  test identities with zero Rjest-only failures, but not those unmeasured
...
- Synchronous, async-only, and disagreeing dual-hook custom resolvers work
  ...
  React Native resolver are covered separately. The rest of the current Jest
  resolver option surface, pnpm layouts, other PnP fallback modes, and broader
  custom-resolver/PnP combinations remain open.
...
2. Add a pnpm-layout corpus and broaden configured-resolver option coverage.
