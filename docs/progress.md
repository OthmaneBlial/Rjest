...
- An automated differential denominator containing both passing probes and
  preserved known incompatibilities. The current generated result is 70/71
  compatible scenarios (98.6%), with ESM at 7/7 (100.0%), transforms at 5/5
  (100.0%), mocks at 9/10 (90.0%), and per-category scores in the machine-readable
...
corpus score is 70/71 (98.6%). ESM is 7/7 (100.0%), transforms are 5/5
(100.0%), mocks are 9/10 (90.0%), and coverage is 3/3 in their bounded scenario
categories.
...
- Deeply unmocked dependency graphs and `jest.onGenerateMock` are not
  implemented. Manual CommonJS/native-ESM mocks and CommonJS/native-ESM
  automocking, Babel-hoisted and virtual factories, and direct/transitive
  synchronous or asynchronous ESM module factories are covered by differential
  tests. ESM unmock/reset behavior and asynchronous CommonJS/ESM isolated
  registries are covered.
...
2. Grow the versioned differential denominator from deterministic real-project
   failures.
3. Implement `jest.onGenerateMock` for CommonJS and native ESM automocks.
